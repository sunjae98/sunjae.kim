      ...
      <div className={`${webMenu.common} ${webMenu.web} ${webMenu.mobile}`}>
        {categories.map((category, index) => (
          <Link key={index} href={`/${category.replace(/\s+/g, '').toLowerCase()}`} passHref>
            <div className="font-sans text-2xl">{category}</div>
          </Link>
        ))}
      </div>
      {/* 햄버거 메뉴 버튼 (모바일 뷰) */}
      <button
        className="md:hidden flex flex-col h-10 w-10 border-none rounded justify-center items-center group bg-[#3a393a]"
      ...
      {isMenuOpen && (
        <div
          ...
          <div className={`fixed top-16 right-0 w-3/4 h-full bg-background-light pl-4 z-50`}>
            ...
            {categories.map((category, index) => (
              <Link key={index} href={`/${category.replace(/\s+/g, '').toLowerCase()}`} passHref>
                <div className="flex align-bottom font-sans text-2xl pt-3 pl-3 h-16 border-b-4 border-background-theme">
                  {category}
                </div>
              </Link>
            ))}
